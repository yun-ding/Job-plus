...
public class GitHubClient {
	...
	public List<Item>  search(double lat, double lon, String keyword) {
	    ...
	    ResponseHandler<List<Item>> responseHandler = new ResponseHandler<List<Item>>() {
	        ...
	        public List<Item> handleResponse(
	            ...
	            if (status != 200) {return new ArrayList<>();}
	...
	private List<Item> getItemList(JSONArray array){
		...
		for (int i=0; i<array.length(); i++) {
			//extract keywords from description since Github API doesn't return keyword
		...
		//get keywords from nultiple text in one request since
