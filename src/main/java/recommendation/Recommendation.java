	...
	public List<Item> recommendItems(String userId, double lat, double lon){
		...
		for (String itemId : favoritedItemIds) {
			...
			for (String keyword : keywords) {
				//if allKeywords contains keyword then count++
				//it not then put keyword into allKeywords (count at least == 1)
		...
		//lambda comparator
		//how to sort: compare e1.getValue() and e2.getValue()
		...
		for (Entry<String, Integer> keyword : keywordList) {
			List<Item> items = client.search(lat, lon, keyword.getKey());
  			//avoid duplicate items in favorite list
			//and because mutiple favorite-query, avoid visiting same items from related keywords
			for (Item item : items) {
